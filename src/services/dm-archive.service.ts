...
import {
  ...
  DM_QUOTA_MAX_DAYS,
  DM_PURGE_INACTIVE_THRESHOLD,
  DM_PURGE_INACTIVE_KEEP,
  DM_P2P_CACHE_TTL,
} from '../types/dm-archive';
import type {
  DMConversationStats,
  DMArchiveEntry,
  DMArchivePushEvent,
...
export class DMArchiveService {
   ...
   */
  async getConversationStats(conversationId: string): Promise<DMConversationStats> {
    ...
    const [archRows] = await this.db.query(
      `SELECT COALESCE(SUM(messages_archived), 0) as archived_count 
       FROM dm_archive_log WHERE conversation_id = ?`,
      [conversationId]
    );
    ...
    await this.db.execute(
      `INSERT INTO dm_conversation_stats 
   ...
   */
  async checkQuota(conversationId: string): Promise<{
    exceeded: boolean;
    ...
    if (stats.messageCount > DM_QUOTA_MAX_MESSAGES) {
      const excess = stats.messageCount - DM_QUOTA_MAX_MESSAGES;
      return {
        exceeded: true,
    ...
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - DM_QUOTA_MAX_DAYS);

    const [oldRows] = await this.db.query(
      `SELECT COUNT(*) as count FROM messages 
       WHERE conversation_id = ? AND created_at < ? AND is_deleted = FALSE`,
   ...
   */
  async getArchiveStatus(conversationId: string): Promise<DMArchiveStatus> {
    const stats = await this.getConversationStats(conversationId);
    const quotaPercent = (stats.messageCount / DM_QUOTA_MAX_MESSAGES) * 100;
  ...
  async archiveOldMessages(
  ...
  ): Promise<DMArchivePushEvent | null> {
    ...
    const archiveEntries: DMArchiveEntry[] = messages.map(msg => ({
      messageId: msg.id,
    ...
    await this.db.execute(
      `INSERT INTO dm_archive_log (id, conversation_id, messages_archived, oldest_message_at, newest_message_at, reason)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [
        archiveLogId,
        conversationId,
    ...
    await this.redis.set(
      `dm:archiving:${conversationId}:${archiveLogId}`,
      JSON.stringify(messageIds),
      3600 // 1h timeout pour la confirmation
    );
   ...
   */
  async confirmArchive(conversationId: string, archiveLogId: string, peerId: string): Promise<boolean> {
    ...
    if (confirms.length >= participantIds.length) {
      ...
      if (messageIds.length > 0) {
        ...
        logger.info(`🗑️ ${messageIds.length} MP supprimés de la DB après confirmation P2P (${conversationId})`);
      }
   ...
   */
  async getCachedArchivedMessage(messageId: string): Promise<DMArchiveEntry | null> {
   ...
   */
  async cacheArchivedMessage(message: DMArchiveEntry): Promise<void> {
    await this.redis.set(
      `dm:cached:msg:${message.messageId}`,
      JSON.stringify(message),
      DM_P2P_CACHE_TTL
    );
    logger.info(`💾 MP ${message.messageId} mis en cache Redis (24h)`);
  }

  /**
   * Met en cache plusieurs messages récupérés d'un peer
   */
  async cacheBulkArchivedMessages(messages: DMArchiveEntry[]): Promise<void> {
   ...
   */
  async purgeInactiveConversations(): Promise<{
    ...
    const archiveEvents: DMArchivePushEvent[] = [];
    ...
    logger.info(`🧹 Purge intelligente: ${purgedConversations} conversations, ${totalMessagesPurged} MP archivés`);
   ...
   */
  async checkAndArchiveAfterCreate(conversationId: string): Promise<DMArchivePushEvent | null> {
  ...
  async runDailyMaintenance(): Promise<{
    ...
    logger.info('🔧 Début maintenance quotidienne DM...');
    ...
    const [dmConversations] = await this.db.query(
    ...
    for (const conv of dmConversations as any[]) {
    ...
    logger.info(`🔧 Maintenance terminée: ${purgeResult.purgedConversations} purges, ${ageArchiveCount} MP archivés par âge`);

    return { purgeResult, ageArchiveCount };
  }
}

export const dmArchiveService = new DMArchiveService();
