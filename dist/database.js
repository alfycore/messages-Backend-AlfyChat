...
async function runMigrations(db) {
    ...
    try {
        const [idxRows] = await db.execute(`SELECT COUNT(*) as cnt FROM information_schema.statistics WHERE table_schema = DATABASE() AND table_name = 'messages' AND index_name = 'idx_messages_conv_created'`);
    ...
    try {
        const [rows] = await db.execute(`SELECT COUNT(*) as cnt FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'reactions'`);
        if (rows[0]?.cnt > 0) {
